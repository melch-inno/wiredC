...
import { validateRequest, requiresUser, checkRole } from "./middleware";
import {
  createUserSchema,
  updateUserSchema,
...
export default function (app: Express) {
  app.get("/healthcheck", (req: Request, res: Response) => res.sendStatus(200).json({message: "OK"}));

  app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, '/static/index.html'));
  });

  app.get('/api/auth', oauthHandler);
  ...
  app.put("/api/user/delete/:userId",
    checkRole,
    deleteAndReactivateUserHandler
  );
  ...
  // Delete a post
  app.delete(
    "/api/posts/:postId", [checkRole, validateRequest(deletePostSchema)], deletePostHandler);
}
